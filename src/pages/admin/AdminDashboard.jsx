...
import { getAdminDashboard, adminCancelAppointment } from "../../api/api.js";
import { LoaderCircle, Users, Stethoscope, CalendarDays, X } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const stats = [
    { label: "Doctors", value: data?.totalDoctors ?? 0, icon: Stethoscope, color: "text-[#5F6FFF]", bg: "bg-[#F2F3FF]" },
    { label: "Appointments", value: data?.totalAppointments ?? 0, icon: CalendarDays, color: "text-blue-500", bg: "bg-blue-50" },
    { label: "Patients", value: data?.totalUsers ?? 0, icon: Users, color: "text-gray-600", bg: "bg-gray-100" },
  ];
        ...
        <div className="divide-y">
          ...
          {data?.latestAppointments?.map((apt) => (
            <div key={apt._id} className="flex items-center gap-4 px-6 py-4">
              <img
                src={apt.doctor?.image || `https://ui-avatars.com/api/?name=${apt.doctor?.name}&background=EEF2FF&color=5F6FFF`}
                alt=""
                className="w-10 h-10 rounded-full object-cover bg-[#EAEFFF]"
              />
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-800 truncate">{apt.doctor?.name}</p>
