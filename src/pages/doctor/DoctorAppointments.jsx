...
import { getDoctorAppointments, markAppointmentComplete, doctorCancelAppointment } from "../../api/api.js";
...
const DoctorAppointments = () => {
  ...
  const handleComplete = async (id) => {
    try { await markAppointmentComplete(id); fetchAppointments(); } catch (err) { console.error(err); }
  };

  const handleCancel = async (id) => {
    try { await doctorCancelAppointment(id); fetchAppointments(); } catch (err) { console.error(err); }
  };
  ...
  return (
    <div>
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        <div className="hidden sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_2fr_1fr_1fr] gap-2 px-6 py-3 border-b bg-gray-50 text-sm font-medium text-gray-500">
          <p>#</p>
          <p>Patient</p>
          <p>Payment</p>
          <p>Age</p>
        ...
        <div className="divide-y">
          ...
          {appointments.map((apt, index) => (
            <div key={apt._id} className="flex flex-col sm:grid sm:grid-cols-[0.5fr_2fr_1fr_1fr_2fr_1fr_1fr] gap-2 items-start sm:items-center px-6 py-4 text-sm text-gray-600 hover:bg-gray-50 transition-all">
              ...
              <span className="border border-gray-300 text-xs px-2 py-0.5 rounded-full font-medium">
                {apt.paymentStatus === "paid" ? "ONLINE" : "CASH"}
              </span>
